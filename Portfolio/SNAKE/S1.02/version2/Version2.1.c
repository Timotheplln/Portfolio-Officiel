...
int main(){
    ...
    do{
        ...
        if(pomme[X_TAB]<lesX[0]){
            if((lesX[0]-pomme[X_TAB])>(MAX_LONG-pomme[X_TAB]+MAX_LONG-lesX[0])){
                if(lesY[0]>(MAX_HAUT/2)+1){
                    if(direction==BAS){
                        if(lesX[0]!=1){
                            direction=GAUCHE;
                        }else{
                    ...
                    direction=HAUT;
                }else if(lesY[0]<(MAX_HAUT/2)-1){
                    if(direction==HAUT){
                        if(lesX[0]!=MAX_LONG-1)
                        direction=DROITE;
                        progresser(lesX,lesY,direction, &collision, taille, pomme, &mangee);
                    }
                    direction=BAS;
                }
            }else if((lesX[0]-pomme[X_TAB])<(MAX_LONG-pomme[X_TAB]+MAX_LONG-lesX[0])){

            }
        }else if(pomme[X_TAB]>lesX[0]){
            if((pomme[X_TAB]-lesX[0])>(MAX_LONG-lesX[0]+MAX_LONG-pomme[X_TAB])){
                if(lesY[0]>(MAX_HAUT/2)+1){
                    if(direction==BAS){
                        direction=GAUCHE;
                        progresser(lesX,lesY,direction, &collision, taille, pomme, &mangee);
            ...
            }else if((pomme[X_TAB]-lesX[0])<(MAX_LONG-lesX[0]+MAX_LONG-pomme[X_TAB])){
