...
void progresser(int lesX[TAILLE_MAX], int lesY[TAILLE_MAX], char direction, bool *collision, int taille, int destination[2], bool *mangee, int *etape); // Met à jour les positions du serpent et vérifie les collisions
...
int main(){
    ...
    for(i=0;i<NB_PAVES;i++){ // Génération des pavés sur le plateau
        for(j=0;j<TAILLE_PAVE;j++){
            for(k=0;k<TAILLE_PAVE;k++){
                afficher(lesPavesX[i]+j,lesPavesY[i]+k,BORDURE); // Afficher le pavé
            }
        }
    }
    ...
    do{
        if(meilleurC==calcul[0]){
            etape=1;
            progresser(lesX,lesY,direction, &collision, taille, pomme, &mangee, &etape);
        }else if(meilleurC==calcul[1]){
            destination[X_TAB]=PORTAILD[X_TAB];
            destination[Y_TAB]=PORTAILD[Y_TAB];
            if(etape==0){
                progresser(lesX,lesY,direction, &collision, taille, destination, &mangee, &etape);
            }else{
                progresser(lesX,lesY,direction, &collision, taille, pomme, &mangee, &etape);
            }
...
*/
void progresser(int lesX[TAILLE_MAX], int lesY[TAILLE_MAX], char direction, bool *collision, int taille, int destination[2], bool *mangee, int *etape){
    ...
    if(  ((lesX[0]<=MIN) && ((lesY[0]<((MAX_HAUT/2)-TAILLE_TROU)) || (lesY[0]>=((MAX_HAUT/2)+TAILLE_TROU))))  ||  ((lesY[0]<=MIN) && ((lesX[0]<((MAX_LONG/2)-TAILLE_TROU)) || (lesX[0]>=((MAX_LONG/2)+TAILLE_TROU))))  ||  ((lesX[0]>=MAX_LONG) && ((lesY[0]<((MAX_HAUT/2)-TAILLE_TROU)) || (lesY[0]>=((MAX_HAUT/2)+TAILLE_TROU))))  ||  ((lesY[0]>=MAX_HAUT) && ((lesX[0]<((MAX_LONG/2)-TAILLE_TROU)) || (lesX[0]>=((MAX_LONG/2)+TAILLE_TROU))))  ){ // Vérifier si le serpent a touché la bordure
        *collision=true;
    }
